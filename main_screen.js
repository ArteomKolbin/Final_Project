...
const MainScreen = ({ navigation }) => {
  ...
  const [clickMultiplier1, setClickMultiplier1] = useState(-1); 
  const [clickMultiplier2, setClickMultiplier2] = useState(-10); 
  useFocusEffect(
    React.useCallback(() => {
      loadScore();
  ...
  const handleClick1 = () => {
    if (canClick) {
      const newScore = score + clickMultiplier1;  // Увеличиваем очки в зависимости от множителя
      saveScore(newScore);
    }
  };
  ...
  const handleClick2 = () => {
    if (canClick) {
      const newScore = score + clickMultiplier2;  // Увеличиваем очки в зависимости от множителя
      saveScore(newScore);
    }
  };
 ...
 <Text style={styles.buttonText}>Кликни!</Text>
      ...
      >
        ...
        <TouchableOpacity
        style={styles.clickButton2}
        onPress={handleClick1}
        disabled={!canClick}
      ><Text style={styles.buttonText}>Не кликай!</Text>
      </TouchableOpacity>
